...
def get_greatest(number_list):
    ...
    for i in range(1, len(number_list)):
        if maxValue < number_list[i]:
            maxValue = number_list[i]
    greatest_number = maxValue
...
def get_smallest(number_list):
    ...
    주어진 리스트에서 제일 작은 숫자를 반환함
        ...
        Examples:
            ...
            11
    """
    minValue = number_list[0]
    for i in range(1, len(number_list)):
        if minValue > number_list[i]:
            minValue = number_list[i]
    smallest_number = minValue
...
def get_median(number_list):
    ...
    if len(number_list) % 2 == 0:
        median = (number_list[half-1] + number_list[half]) / 2
    else:
        median = number_list[int(half)]
    return median
